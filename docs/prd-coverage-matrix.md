...
**Owned by [`PF-816`](../tickets/plugforge/lane-26-submission-artifacts.md) (L26).** Until
2026-08-15 this file was referenced by **no ticket in any lane** — an unowned graded artifact,
which is how its flag-matrix row stayed **WEAK** on a grep that had stopped being true. PF-816
carries the standing obligation to re-verify it against `origin/main` before submission.

**Measured 2026-08-15 against `origin/main`.** Rows re-verified in the 2026-08-15 reconciliation
pass are pinned to **`a96cdda`** and name the job id that proves them; rows untouched by that pass
still carry their earlier `94a6905` measurement. One row per PRD requirement, read out of the
eighteen per-page extracts in `.claude/prd/page-N.txt` — never out of `full.txt`, which reflows
and has produced misattributed citations before.
...
| Public/internal boundary: lint rule fails the build on cross-import | `eslint.config.js:154` (fence 1/2), `:198-223` (fence 3, `integrations/` → only `@ship/sdk`), fixtures under `eslint-fixtures/` | SATISFIED | CI job `boundary-lint` **68246**, green on pipeline **#20358** (`main`, `a96cdda`) |
...
| TTFE < 60 s in CI (p.6, p.8) | **18.9 s** job wall clock; **7 001 ms** drill total | **YES** | **Upgraded to `main` evidence 2026-08-15.** GitLab job **68256**, pipeline **20358**, ref **`main`**, sha `a96cdda`: `Test Files 1 passed (1)`, `Tests 3 passed (3)`, drill instrumentation `TOTAL 7001 ms` against `budget 60000`, `ttfe: job wall clock 18.9 s`, `ttfe series check OK`; `ttfe-controls` (**68257**) green beside it. Supersedes job 66739 / pipeline 20237 on `pf/L20-ttfe-ci-docker` (56.374 s), which was a feature branch — this is the graded branch and a stronger claim |
...
| Drill flake rate 0% over 20 consecutive CI runs (p.9) | **20/20 — 0% flake** | **YES** | **Upgraded to `main` evidence 2026-08-15:** GitLab job **68258** (`ttfe-soak`), pipeline **20358**, ref **`main`**, sha `a96cdda` — `ttfe soak: 20/20 passed`, `pass rate 20/20`. Supersedes job 67859 / pipeline 20338 on `pf/L20-flake-and-clean` (same result, feature branch). `check-series.mjs --soak` gates it and refuses a window that is not 20 runs of exactly one commit. **Read the shape of the claim:** twenty consecutive drill runs *inside one CI job*, not twenty separate pipeline runs — an accumulated window would span twenty commits, which `--soak` rejects by design, and this runner has no shared cache to carry a series between pipelines. All 20 samples are above F80's load veto (`load-certified 0/20`), which does not weaken a flake count — contention makes flake likelier, so 20/20 under load is the stronger result — but does mean the 8500 ms P95 beside it is not a certified platform timing. `docs/ttfe-drill.md` → *"The 20-run soak"* |
...
| **item 8** — the agent rewire lands *"behind a feature flag so Part 2's tests pass with the flag on or off"* (p.11:23); **p.17 §2.6** asks *"How does CI prove Part 2's tests pass with the flag both on and off?"* | The flag is `SHIP_AGENT_VIA_SDK`. `scripts/agent-flag-matrix.sh` (PF-706) runs the flag-invariant suite in **both** states, both blocking, with two anti-vacuity guards — each leg asserts a minimum test count, and both legs must run the same file set. `docs/l23-flag-matrix.md` is a measured three-bucket inventory and states the limit plainly: passing in both states is true *"at the suite level, with one e2e assertion forked and named, and one composition-root spec running in its own state"*, **not** byte-for-byte | **SATISFIED** |
| | **UPGRADED WEAK → SATISFIED, 2026-08-15. This row understated us; the wiring landed and the row was never re-run.** It previously read *"CI does not answer it … returns **no match** across all **31** jobs."* Both halves are now false. The same grep, run verbatim at `a96cdda` — `grep -nE "agent-flag-matrix\|SHIP_AGENT_VIA_SDK" .gitlab-ci.yml .github/ -r` — returns **six** lines, of which `.gitlab-ci.yml:462` is the job itself and `:469` is `- ./scripts/agent-flag-matrix.sh`, `allow_failure: false`. The pipeline carries **28** jobs, not 31 (32 top-level keys in `.gitlab-ci.yml` less `workflow`, `stages`, `default`, `variables`; pipeline 20358 enumerates 28). **And it has run green on `main`:** job **68255**, pipeline **20358**, sha `a96cdda`, ref `main` — `ok leg off: 230/230`, `ok leg on: 230/230`, `PF-706 ok — bucket 1 is green in BOTH states, 230 tests per leg`. So p.17 §2.6's question — *"How does CI prove Part 2's tests pass with the flag both on and off?"* — now has a CI answer, on the graded branch, and the p.11 half was never in doubt. **The one limit worth stating plainly, so this is not read as more than it is:** the matrix runs the **flag-invariant bucket**; bucket 3's single conflicting e2e assertion (`e2e/fleetgraph-agent.spec.ts:431-443`) is named and excluded, not forked — see PF-707, whose criterion was corrected in the same pass for claiming a fork that does not exist. | |
...
|---|---|---|
| 1 | ~~**Flag matrix proves nothing in CI** (p.17 §2.6)~~ | **DONE — verified green on `main` 2026-08-15.** The job exists (`.gitlab-ci.yml:462`, `allow_failure: false`) and ran as job **68255** on pipeline **20358** at `a96cdda`: `ok leg off: 230/230`, `ok leg on: 230/230`. The "zero hits across all 31 GitLab jobs" line was stale — the grep returns **six** lines and the pipeline has **28** jobs. |
...
| 3 | **Three** failing jobs on the graded pipeline (was "nine") | **Re-measured on pipeline 20358 (`main`, `a96cdda`) 2026-08-15.** The failures are `test` (**68253**), `coverage` (**68265**) and `regression-budget` (**68252**) — three, not nine. **Neither job this row named as trivially green-able is still failing:** `type-violations` (**68248**) and `terraform-verify` (**68267**) are both **green**. `test` and `coverage` are the known `staticCopy.test.ts` breakage (it writes the committed `docs/openapi.json` and its import list omits the audit route), owned by another agent. `e2e` (**68266**) is red but **non-blocking** (`allow_failure: true`) — 887 passed / 1 failed of 888, the failure being a Week-5 a11y assertion at `accessibility-remediation.spec.ts:1112`, not a PlugForge regression — so it is disclosed but not counted among the blocking three. |
