...
| 3 | **Three** failing jobs on the graded pipeline (was "nine") | **Re-measured on pipeline 20358 (`main`, `a96cdda`) 2026-08-15.** The failures are `test` (**68253**), `coverage` (**68265**) and `regression-budget` (**68252**) — three, not nine. **Neither job this row named as trivially green-able is still failing:** `type-violations` (**68248**) and `terraform-verify` (**68267**) are both **green**. `test` and `coverage` are the known `staticCopy.test.ts` breakage (it writes the committed `docs/openapi.json` and its import list omits the audit route), owned by another agent. `e2e` (68266) was still running at the time of this measurement and is not counted. |
