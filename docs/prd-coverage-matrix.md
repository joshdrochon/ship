...
|---|---|---|
| 1 | **Per-slice PR descriptions at scale** (p.12, third clause) | Seven slice-branch MRs exist (!21–!27) and five are compliant, so the practice is demonstrated — but it cannot be applied retroactively to 185 slices. Opening ~180 MRs after the merges they describe would fabricate a paper trail. State the seven, concede the *"each"*. |
