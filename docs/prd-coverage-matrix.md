...
**Bottom line: 85 requirement rows — 76 SATISFIED, 7 WEAK, 2 MISSING.** *(Moved 2026-08-16, second edit: **two** rows WEAK → SATISFIED — the p.2 TF row and p.5's drift/destroy row — because destroy-redeploy **drill 2** ran and the re-apply succeeded unaided, which is the clause both rows were WEAK on. The count is +2/−2 on those two rows against the figure below and **nothing else was recounted**; a full re-parse is not claimed. Earlier the same day: **TS-9 — TTFE drill from a clean container**, WEAK → SATISFIED, +1/−1. Two WEAK rows that gained evidence this pass **stayed WEAK on purpose** — p.12's PR-description clause and TS-6's literal "within 2s".)* The two MISSING are
