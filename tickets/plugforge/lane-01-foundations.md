...
| PF-030 | ☑ The api suite passes consecutively — MVP gate item 9 requires it and today it does not | Measured, five consecutive runs, no code change between them: `1887 pass · 1 fail (audit/orderAndDurability) · 1887 · 1887 · 3 fail (api/v1/pagination)`. Different file each time. Three causes in this family have already been found and fixed (F46's module-level `apiLimiter` budget, L11's `Date.now()+50` busy-wait truncating to 49.x ms, `public_api_calls` missing from the test TRUNCATE list) and it still flakes, so at least one remains. `fileParallelism: false` is already set. Acceptance: the cause is **named** and fixed — not retried, not `test.retry()`, not reordered until it hides — and ten consecutive full-suite runs pass. p.9 states the standard for the drill and it is the right one here: *"any flake = bug in the drill or the platform."* Same family as F33/F44/F46 **REOPENED 2026-08-13.** I closed this on ten consecutive clean runs and it has failed **twice since**, both times `__tests__/internal-limiter-scope.test.ts` PF-215. L05 independently filed **F49** for the sibling file `platform/ratelimit/boundaryAndOrder.test.ts` — measured on clean `pf/integration` before it wrote any code, ~1-in-3 under load, passes alone. **The two files that flake are the only two that set `API_RATE_LIMIT_MAX=1`.** The ten clean runs happened on an idle machine; both failures happened while parallel agents were saturating it, which is why ten runs were not the proof I treated them as — I should have run them under the load the failures appear under. PF-222 is genuinely fixed and stays fixed; this is the separate residual. Standing hypothesis, untested: `apiLimiter`'s window is wall-clock 60 s, so a file that runs slower under contention crosses the boundary mid-file and an assertion expecting an exhausted bucket meets a fresh one. p.11 forbids exactly this shape. Revised acceptance: the cause is **named**, the fix injects a store or a clock rather than reordering assertions, and ten consecutive runs pass **while three parallel builds are running** — not on an idle machine. **UPDATE — three causes found and fixed, one residual.** (1) `setup.ts`'s `afterAll` never closed the pg pool; its comment claimed `globalTeardown` did, and there is no `globalTeardown` in `api/vitest.config.ts`. 145 isolated module graphs meant 145 open pools, idle backends held locks, and the `beforeAll` TRUNCATE queued past vitest's 10 s hook timeout — killing whole FILES, 23 in one run and 32 in another. The summary reports that as failing tests, which is why it read as "a different random test each run" for days; it was files that never started. (2) PF-215's `write EPIPE`: PF-214 spends the `API_RATE_LIMIT_MAX=1` budget, the 2 MB POST then 429s, express short-circuits without draining, and the client writes into a socket nobody reads. (3) PF-305's `socket hang up`: 51 sequential `request(app)` calls are 51 listen/close cycles. All three are RACES, which is exactly why all three passed in isolation — in isolation there is nothing to race against. **Residual:** 2 failures in 9 full-suite runs, `deviceCodes` PF-123 and `documents.pagination` PF-257, both passing 5/5 in isolation in ~2 s and both suspected load-sensitive timeouts rather than logic. Ten consecutive clean runs is NOT yet met, so this stays open. **Verified 2026-08-13:** Suite 160/160 files green. Latency half resolved: the recorded baseline was not reproducible on this machine (the baseline COMMIT itself measured 1.5-3x its own recorded numbers today), so the denominator was re-captured at the true baseline commit 41393f6 in the same session as the numerator. baseline:compare --strict-latency now reports WITHIN BUDGET, 13 enforced metrics at or under +10%, latency ENFORCED. | MVP-9 | p.2, p.9 | — |
