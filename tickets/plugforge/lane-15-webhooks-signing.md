...
### S3 · `pf/L15-signer` — PF-434 – PF-439

47 cases in `api/src/platform/webhooks/signer.test.ts`. Zero `setTimeout`, zero
wall-clock reads — asserted by the file greping itself.

| Ticket | Proof |
|---|---|
| PF-434 | `positive` → `signedPayload(7, "abc")` is byte-equal to `"7.abc"`; `negative` → a signature computed over the **body alone** does not verify, and a signature for a different `t` does not verify at this one; `tamper` → a rewritten `t` fails, which is the whole point of signing it. Rationale plus both rejected alternatives written into `docs/architecture.md` ("What is signed, and why the secret is encrypted rather than hashed") |
| PF-435 | `SIGNATURE_HEADER_PATTERN` is exported and the emitted header is asserted against it, plus separately: seconds not milliseconds (`1715985600`, p.7's own example second), no whitespace anywhere, exactly one comma, `t` before `v1`. The parser returns `null` — never throws — for **12** malformed shapes, and for a **duplicated key**, which a `Map` parse would have resolved last-wins and handed an attacker the replay window back. An unknown `v2=` pair is tolerated, which is the forward-compatibility half of PF-434's "the scheme tag lives in the header" |
| PF-436 | `DeliveryRequest.rawBody` is now a `Buffer`, and `envelopeToRawBody` returns one — the bytes MACed and the bytes sent are the same object, so there is no overload that could re-serialize. `tamper` → a REORDERED but JSON-equivalent body fails verification while `JSON.parse` of both is `toEqual`, which is the canonicalization bug stated as a passing negative. Non-ASCII, emoji and `/` round-trip |
| PF-437 | `SignatureSigner` takes `Pick<Clock,'nowMs'>` as a **constructor argument with no default**; a comment-stripped grep asserts `Date.now(`, `new Date(`, `setTimeout` and `setInterval` appear nowhere in `signer.ts`; `Math.floor` not rounding is pinned (9.999 s is second 9); the same clock twice produces byte-identical headers, which is what makes PF-446's vectors committable |
| PF-438 | All five ticketed assertions plus more: valid passes, tampered body fails, stale `t` fails, `t` **further in the future** than tolerance fails (`Math.abs` pinned from the future side), missing `v1` fails (p.4). `timingSafeEqual` over equal-length buffers with a length check first. `DEFAULT_TOLERANCE_SECONDS` is 300 and is asserted **behaviourally** as the default, not just as a constant |
| PF-439 | Four `describe` blocks named `positive`, `negative`, `replay`, `tamper` — the PRD's own four words, p.11. A fifth block reads this file and asserts each of the four contains more than three `expect(` calls, so the suite cannot rot into four empty describes that read as coverage. Beyond the four: wrong secret of the correct length, one flipped hex character, one trailing newline, and an empty body signing and verifying |

**Seam change L16 must know about:** `DeliveryRequest.rawBody` is `Buffer`, not `string`,
and the interface gained `signedAtSeconds`. POST the buffer directly;
`rawBody.toString('utf8')` is correct for the delivery log. A
`JSON.stringify(JSON.parse(...))` anywhere on the delivery path re-introduces exactly the
bug PF-436 exists to prevent.

### S4 · `pf/L15-pipeline-matcher` — PF-440 – PF-443, and **decision D7's gate**

22 cases in `api/src/platform/webhooks/pipeline.test.ts`, every one through the REAL
`InProcessEventBus`. Zero `setTimeout`; the retry-ladder cases advance a `FakeClock`.

| Ticket | Proof |
|---|---|
| PF-440 | Three active subscriptions on one event produce three jobs with three distinct `subscription_id`s and three **distinct signatures over byte-identical bodies** — and each verifies under its OWN secret and fails under both others. Zero matches → zero jobs, `publish()` resolves, and **no error-level log** (asserted against a captured logger, because an error per document create in an unsubscribed workspace would bury the real ones). Other event types and other workspaces asserted absent |
| PF-441 | A deliverer that NEVER resolves: `publish()` still returns, the elapsed time is under the ticket's 200 ms bound, and — the assertion that cannot flake — the delivery is still pending and its promise never settled, so the wire was never inside `publish()`. The handed-over request is fully signed at hand-off (header matches the grammar, verifies under the subscription's secret). A THROWING deliverer neither rejects `publish()` nor crashes the process on an unhandled rejection. A decryption failure delivers nothing and the log names `WEBHOOK_SECRET_KEY` and the word `unsigned` |
| PF-442 | Attempt 2 via `resign()` after advancing the clock 4 s: `signedAtSeconds` is +4, `signatureHeader` differs, and `rawBody` (`Buffer.compare === 0`), `eventId` and `idempotencyKey` are identical — all five relations in one pass. Plus the reason it is not cosmetic: after advancing 31 minutes (past the 300 s window, inside the p.4 retry tail) attempt 1's header **fails** verification and attempt 2's **passes**. `resign()` returns `null` once the subscription is deactivated, so L16 abandons rather than dead-letters |
| PF-443 | Attempt 1 signed with secret A; `rotateSecret` between attempts; attempt 2 verifies under B and **fails** under A. Second assertion: `rawBody`, `eventId`, `idempotencyKey`, `subscriptionId` and `targetUrl` are all unchanged — rotation changes the signature, never the message |
| **D7 gate** | A workspace-visible document goes to every match. A private document reaches **only** the subscription whose `user_id` equals `data.created_by` — asserted against three subscriptions (author, another user, machine-to-machine). It works for `document.deleted`, where finding F10 leaves **no row to read**, which is the case that forced the design. An 11-row unit table over `gateSubscription`, including the case that must not pass by accident: `null === null` is true in JavaScript, so `{visibility:'private', created_by:null}` with `user_id:null` must be `private-not-owner` and is. An unrecognisable `visibility` is a distinct outcome, warned about by name, and delivered to nobody |

**Decision recorded, and it is a narrowing of D7:** the *"or that user is a workspace
admin"* clause is **not** implemented and the gate fails closed without it. It needs a
`workspace_memberships` read per (event × subscription) on the request path — the exact
cost the gate exists to avoid — and "an admin subscribed a webhook, therefore every
private document in the workspace is fanned out to that URL" is a surprising default
nobody asked for. The conservative reading costs an admin some events they could have
read by hand; the permissive one ships other people's private titles to a third-party
endpoint. A test pins the narrowing so adding it later is a deliberate act with a failing
assertion. If it is wanted, it belongs as an explicit per-subscription opt-in on the row,
not as a consequence of a role.

### S5 · `pf/L15-ts6-server-half` — PF-444 – PF-447

| Ticket | Proof |
|---|---|
| PF-444 | `testingScenario6.test.ts` — the REAL composition root (`createApp(testDeps({...}))`) with the REAL bearer middleware over a real database. Subscribe through `POST /api/v1/webhooks`, `POST /api/v1/documents`, then: **exactly one** job handed off (not "at least one" — a duplicate would mean two publish sites), `targetUrl` matches, the body parses against L14's `eventEnvelopeSchema` with `data.id` equal to the created document and `workspace_id` equal to the token's, and the `Ship-Signature` verifies under the secret returned at **creation**. Two negatives: a write in an unsubscribed workspace produces nothing, and a deactivated subscription's URL never appears |
| PF-445 | Four assertions against the exact bytes PF-444 captured, plus a control that they verify untouched first. **Tamper:** one character inside `data.title` (same length, still valid JSON) → `false`. **Replay:** the identical body and header at +299 s → `true`, at +300 s → `true`, at +301 s → `false`; the boundary is asserted from both sides and the only thing that changed is the presentation time. **p.4's missing `v1`** → `false`. **Wrong secret** on the untouched request → `false`, which is what makes the drift/secret-mismatch differential in the architecture doc operationally checkable |
| PF-446 | `__fixtures__/signature-vectors.json` — **8** committed vectors, each with a `note` saying what it is for. Covers a real envelope, an **empty** body, **non-ASCII** (multi-byte, em dash, astral-plane emoji), a body carrying the header's **own delimiters** including a decoy `t=`/`v1=` pair, an unescaped `/`, the same bytes under a **second secret** (headers must differ — the fanout property), timestamp `0` (the value `Number("")` also produces, which is why the parser checks digits) and a year-2100 timestamp. `signatureVectors.test.ts` reproduces every `expectedHeader` byte-for-byte and — anti-vacuity — asserts each vector **fails** under a different secret, so a `verifySignature` that returned `true` unconditionally could not pass |
| PF-447 | `docs/architecture.md` gains "The timestamp: what it defends, the window, and clock drift", answering p.13's three questions: **(a)** capture-and-resend, defeated because `t` is inside the signed bytes, plus the distinction from idempotency that the two are usually conflated on; **(b)** 300 s, Stripe's default, asserted from both sides and symmetric into the future; **(c)** drift — with a **differential table** separating "100% failure across all subscriptions at once" (drift, control is NTP) from "one subscription failing" (a stale secret) from "failures that start only after a delivery has been retrying" (signing at enqueue, which is why we sign per attempt); **(d)** the figure already marks ★ where the signature is computed, and the prose now says so. `architectureDoc.test.ts` latches all four, plus PF-434's two rejected alternatives, PF-422's departure, and that the figure no longer says "hashed at rest" |

The composition-root sketch in `docs/architecture.md` predates the build and named
`HmacSigner`, `WebhookPipeline` and `retries`. Rather than rewrite someone else's sketch,
a paragraph beneath it reconciles each name with what shipped and says plainly which parts
are **not** wired yet (the retry ladder and the delivery log are L16's). Latched.
