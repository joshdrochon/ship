...
| PF-659 | ☑ Payloads and response excerpts are collapsed by default, click-to-reveal, and D7-proof | Pre-Search 2.5 (p.17) asks whether the portal shows payloads *"in full, redacted, or behind a click-to-reveal"* and says to *"Defend the choice"* against 1.4's leakage concerns (p.15). **Decision: click-to-reveal, collapsed by default, for the request payload (PF-475's stored `rawBody`) and for `response_excerpt` alike.** Defense: the portal is the one screen where a screenshot or a screen-share captures *many* payloads at once, and `response_excerpt` is a third party's response body that we never controlled. Deliberately **independent of D7** (L99, webhook payload contents, L14 PF-408, marked re-litigate): if D7 lands on ids-only the reveal is harmless and costs one click; if it keeps `title` the default-collapsed state is the difference between a leak and a click. Either way the portal does not change. Test: the payload string is absent from the DOM until the control is activated **Criterion rewritten in place to as-built 2026-08-15; the original premise was wrong and the reason is recorded here.** The premise was that the portal could show the request payload at all. It cannot, and not because the work was skipped: `raw_body` is deliberately absent from the public delivery contract. `api/src/platform/webhooks/deliveryLog.ts:458` — *"Strips `raw_body` and `app_id`. The body is fetched deliberately, never leaked."* — and `deliveries.schema.ts:12-20` is `.strict()`, so a handler that added the field would fail to serialise rather than quietly widen the public contract. `deliveries.routes.test.ts:424` (*"raw_body is nowhere on the wire"*) locks it. The portal consumes `WebhookDelivery` from that public API like any other client, so satisfying the original wording would have required **widening the public contract the schema exists to hold shut** — turning a paginated delivery page into a bulk event export. The shipped code took the safe path; the criterion text was simply never updated to say so, which is the defect being corrected. **As-built acceptance:** two `RevealPanel` controls in `web/src/components/portal/DeliveryDetailPanel.tsx` — `:102` for `response_excerpt` and `:107` for `signature_header` — both collapsed by default, both revealing only on activation, with the excerpt string *"upstream unavailable"* measured **absent from the DOM** before Reveal. `response_excerpt` is the field that actually carried Pre-Search 1.4's leakage risk here: it is a third party's response body that we never controlled, and it is the one a screen-share captures many of at once. The request payload needs no reveal control because it never reaches the browser — a stronger outcome than click-to-reveal, and the D7-independence argument above is unaffected either way | `CTR:Developer Portal` | p.17, p.15 | PF-658, PF-475 |
...
| PF-662 | ◐ **Testing Scenario 8, portal half** — end to end in Playwright, through the rendered UI | p.5, asserted whole and through the browser rather than the API: seed six consecutive failures so L16 dead-letters the delivery (PF-481 owns the mechanics), open `/portal`, select the app, open the DLQ view, and assert the row is **visible** with `dlq_reason = 'max_attempts_exhausted'`. Point the subscriber at a healthy target, **click Replay in the UI**, and assert the new delivery reaches `delivered` and its `Idempotency-Key` is byte-identical to the original's. Recorded as one test so a partial pass cannot be reported as a pass. The API half is PF-481's; this is the half the PRD's word *"Click"* requires and no API test can cover **STAYS ◐ 2026-08-15 — the spec now EXECUTES, but one named assertion is still absent.** The audit proposed closing this because PF-625 ran the spec; execution is not this criterion. `e2e/portal-replay-ts8.spec.ts` is a single `test()` at `:90` and now genuinely runs — PF-625's `WEBHOOK_SECRET_KEY` fix took it from 2 failed to `1 passed`, so **the spec's own header at `:39` (`STATUS: written, never executed`) is stale and should be corrected by its owner**. Met: six failures seeded (`seedDeadLetteredLadder`), portal opened and app selected, `:162` asserts `max_attempts_exhausted`, `:167` performs the literal UI Replay click, and `:185` asserts `replayed.headers['idempotency-key']` **at the subscriber**, which is stronger than reading our own database back. **Unmet: *"assert the new delivery reaches `delivered`"*** — `grep -n delivered e2e/portal-replay-ts8.spec.ts` returns only `:149` and `:196`, both `toContainText('dead_lettered')`, i.e. the *original* row. The spec proves the replay reached the subscriber and that a new row appeared (`count > 1`), never that its status becomes `delivered`. **Remaining:** one assertion | `TS-8` | p.5, p.4 | PF-661, PF-481, PF-657 |
...
| PF-673 | ☑ Deactivation is reversible, and the UI says so rather than inventing a destructive action | **Criteria rewritten in place to as-built; the original premise was wrong and the S3 correction below records why.** The premise was that `DELETE /:id` removes the row and PF-458's `ON DELETE CASCADE` takes the delivery history with it. It does not: `api/src/platform/api/v1/webhooks/routes.ts:141` documents *"Deactivate a subscription. Idempotent; the row is retained"* and the handler at `:411` calls `repo.deactivate`. There is no destructive verb on the resource, and migration `051_webhook_deliveries.sql:62-68` says the same — the FK cascade exists but the API never triggers it. Shipping the original copy would have made the UI claim an erasure the server does not perform. As-built acceptance: `PATCH /:id` toggles `active`, presented as reversible, stating that pending retries are cancelled (PF-457 closes them as `cancelled`, terminal and distinct from `dead_lettered`); the screen carries a `subscription-lifecycle-note` saying nothing is deleted and the row and its delivery history are retained; and there is **no** destructive control. Tests assert the absence of a `Delete` control and of the word "permanently" on the screen — the anti-vacuity direction, since a passing test for copy that is not rendered proves nothing | `CTR:Webhook Subscriptions` | p.3 | PF-671, PF-431 |
...
| PF-675 | ☑ **Decision:** where the audit-trail view's HTTP surface lives — L12 ships a repository function, not a route | p.4 requires the public audit trail *"Queryable in the developer portal"*, and L12's PF-343 delivers `listCalls({clientId, from, to, status, route, cursor})` — a repository function. React cannot call a repository function, so a route is missing and neither lane has it. **Lean: an owner-scoped route on the internal session surface, gated by app ownership.** Reasons: p.3's seven scopes contain nothing that would gate an audit route on `/api/v1`, and inventing one repeats PF-651's rejected option (c); and p.4 gives Replay an explicit `/api/v1` path while giving the audit trail only *"in the developer portal"*, so the PRD itself does not put it on the public contract. Rejected: an eighth scope plus `/api/v1/audit/calls` (defensible, but it is a PRD extension); reading `public_api_calls` directly from the React page (no). Done when the choice is recorded and the owning lane is named — if L12's audit prefers to own the route, that is a `⚑`, not a local fix **CLOSED ☑ 2026-08-15 — the choice is recorded at length and the owning lanes are named.** `api/src/platform/api/v1/audit/routes.ts:1-92` carries a 92-line header recording the decision (`GET /api/v1/audit`, `scope: null`), why an eighth scope was rejected, why reusing `webhooks:manage` was rejected, and `/api/v1/me` (PF-271) as the precedent; `api/src/routes/apps.ts:366` mounts the session-surface route alongside it. L12 (`listCalls`, PF-343), L22 and F111 are all named. **Correction to this row's own lean, so it does not read as unfollowed:** the lean above was *"an owner-scoped route on the internal session surface"*. What shipped is a **third option this row did not list** — a public `/api/v1/audit` with `scope: null`, *plus* the session route — chosen because p.10's *"eat the dog food"* argues for the public surface and because a second internal data route would trip `portalSurfaceFitness.test.ts`. The criterion asks that a choice be recorded and its owner named, not that the lean be followed, so it is met on its own terms | `CTR:Public Audit Trail` | p.4, p.3 | PF-343 |
| PF-676 | ☐ Audit-call view renders L12's full field set, `request_id` included, and discloses its own traffic | Columns are exactly `PublicApiCallRecord` (PF-326): timestamp, `client_id`, `user_id`, route, scope used, status, latency, **`request_id`** — the field p.18 names and that `docs/architecture.md` omitted until PF-327. Cursor-paginated over PF-343's stable `(occurred_at, id)` ordering, filterable by status and route. A `null` `scope used` renders as "no scope checked" and never as "passed" (PF-333 fixed that meaning; the UI must not re-break it). **Honest disclosure:** the portal's own `/api/v1` reads authenticate as the app being inspected, so they appear as rows in the trail the developer is reading. The view says so and offers a route filter, rather than letting a developer conclude their integration is making calls it never made **STAYS ☐ 2026-08-15 — `AuditPanel.tsx` shipped, but three clauses of this criterion are unmet, including the one it bolds.** The audit that flagged this row as closable was wrong: the file existing is not the criterion. `web/src/components/portal/AuditPanel.tsx` (213 lines) **does** satisfy *"filterable by status and route"* (`:72-98`) and *"cursor-paginated over PF-343's stable `(occurred_at, id)` ordering"* (`usePortalAuditCalls.ts:53-54` walking `client.audit.list({limit, cursor})`, server-side `ORDER BY occurred_at DESC, id DESC` at `api/src/platform/audit/pgAuditSink.ts:175`). **Unmet:** (1) *"Columns are exactly `PublicApiCallRecord`"* — `renderRow` (`:164-179`) emits six spans (`method`, `route`, `status`, `latency_ms`, `scope_used`, `occurred_at`); **`client_id`, `user_id` and `request_id` are not columns** and `method` is added, which is not on the list. `request_id` — the field this criterion bolds — appears only inside the error banner at `:139`. The data is present and mocked (`sdk/src/resources/audit.ts:46-52`, `portalAudit.test.tsx:71-73`); the panel simply does not render it. (2) *"A `null` `scope used` renders as 'no scope checked'"* — `:175` renders `{call.scope_used ?? '—'}`; correctly never *"passed"*, but not the required string either. (3) *"The view says so"* — the self-traffic disclosure lives in the file's doc comment (`:19-23`) and the route header, **never in the UI**; the rendered header (`:65-68`) stops at *"Scoped to this app by its token"*. The route-filter half of clause 3 is met. **Remaining:** three renderer edits, all inside one file | `CTR:Public Audit Trail` | p.4, p.18 | PF-675, PF-326, PF-343 |
| PF-677 | ☑ **The scale answer** — build-cheap vs. rebuild-cheap, decided with the reasoning the question asks for | p.17: *"How will the delivery-log view scale visually when an app has thousands of deliveries — server-side pagination, virtualized list, time-bucket filters? Which is build-cheap and which is rebuild-cheap later?"* The three are not alternatives; they cost differently in both directions. **Server-side pagination is both build-cheap and correct** — PF-464 already returns `{data, next_cursor}` and p.3 mandates that shape, so the portal's cost is consuming it (PF-656), and it is the only one of the three that bounds transfer as well as DOM. **Virtualization is the rebuild-cheap one** — purely client-side, drops into `SelectableList` later without touching the data contract, and is therefore deliberately *not* built now; it only helps once a page holds thousands of rows, which cursor pagination prevents. **Time-bucket filters are the expensive one and the least rebuild-cheap**: they are new server params on PF-226's strict allowlist *plus* an index — PF-463 ships `(subscription_id, attempted_at DESC, id)`, which does not cover a time-range scan across all of an app's subscriptions. Done when this is written into the Pre-Search answer with the index named, and when the deferral of virtualization is recorded as a decision rather than an omission **CLOSED ☑ 2026-08-15 — both clauses are in the Pre-Search answer.** `PRESEARCH-PLUGFORGE.md` §Q37 carries a three-row cost table (build cost now / rebuild cost later / verdict) across all three options. *"With the index named"*, verbatim: *"L16 PF-463's index is `(subscription_id, attempted_at DESC, id)`, which **does not cover a time-range scan across all of an app's subscriptions**."* *"Recorded as a decision rather than an omission"*: the verdict cell reads *"Defer. Being rebuild-cheap is the reason to **not** build it now"*, reinforced by *"Deferring the cheap-to-retrofit thing is not laziness, it is spending the week's hours on the door that closes."* The tracker row marks Q37 **Answered** | `SUB:Pre-Search Document` | p.17, p.3 | PF-656, PF-463 |
...
## Landed — slice S3 (`pf/L22-subscriptions`), 2026-08-15

p.4's *"managing subscriptions"* had no UI at all: verified before building, not assumed — `grep`
over `web/src` found no caller of `client.webhooks.list`, `.create` or `.update`, and the portal's
main content offered only the delivery log. The server side already existed (L15's
`/api/v1/webhooks`, all six methods), so this slice is three components, one hook, and **no new
route** — which is itself the p.10 result: the portal needed nothing added to the public contract
to manage subscriptions, because a stranger's integration needs the same six methods.

**Re-runnable evidence.** Two suites, both green on this branch:

```
web (portal only) : npx vitest run src/components/portal/ src/pages/portal/   64/64 passed (6 files)
web (whole pkg)   : pnpm --filter @ship/web test                            340/340 passed (33 files)
```

`pnpm type-check` clean across all packages; `pnpm lint` 0 errors (289 warnings, none in the files
this slice adds).

| Ticket | State | Evidence |
|---|---|---|
| PF-660 | ☑ | Was ◐ — three of the four states were written and untested. `web/src/pages/portal/portalStates.test.tsx` drives all four through the rendered `PortalPage`: the empty log and the DLQ's own empty state each name what would produce a row; a `ShipError` renders `message` **and** `request_id`; a `kind: 'auth'` failure re-mints **once** (asserted as `invalidatePortalClient` called exactly once, two list calls, and the auth message absent from the DOM) and a **second** auth failure surfaces rather than looping; a 429 renders `Try again in 12s` from `Retry-After` and **disables** Retry. Plus the case that is easy to get wrong: a 502 that also carries `Retry-After` leaves Retry **enabled**, because disabling it would be the portal punishing a user for the server's fault |
| PF-671 | ☑ | `usePortalSubscriptions.ts` → `client.webhooks.list`, `SubscriptionsPanel.tsx` → `SelectableList`, no bespoke table. `portalSubscriptions.test.tsx` asserts the request options are **exactly** `{limit: 25}` — no `offset`, no page number, and no `cursor: null` — and that a server cursor is replayed back verbatim on Next while `next_cursor: null` disables it. `active: false` renders as `inactive` beside an `active` row in the same page, so PF-426's deactivated-is-not-deleted is visible rather than argued. `secret_prefix` and `secret_version` render; the secret does not exist in the read projection at all |
| PF-672 | ☑ | `CreateSubscriptionDialog.tsx`. The event field is a `<select>` over `SHIP_EVENT_TYPES` from `@ship/sdk` — the test asserts `select.options.length === 8` and that the node is a `SELECT`, so an unregistered type is unreachable from the UI as well as rejected by the server. The create call is asserted to send **exactly** `{event, target_url}`. A server `target_url` rejection renders under that field with the form's values intact and `request_id` shown; no PF-425 rule is re-implemented client-side. The returned `signing_secret` goes through **PF-666's `SecretOnceDialog` unchanged** — absent from `document.body.textContent` before Reveal, present after, absent again after Hide |
| PF-673 | ☑ | **With a correction to the ticket, stated rather than quietly worked around.** PF-673 was written expecting `DELETE /:id` to remove the row and cascade the delivery history away. The shipped route is declared *"Deactivate a subscription. Idempotent; the row is retained"* and calls `repo.deactivate` — there is no destructive verb on this resource. Rendering the ticket's warning copy would have been a UI telling a developer their history is about to be erased when the server keeps every row, which is the same class of defect PF-670 exists to prevent. So the panel ships **one** reversible control (Deactivate / Reactivate over `PATCH {active}`) and a lifecycle note stating both true facts. Tests: the toggle sends `{active:false}` then `{active:true}`, the row is **re-read** rather than patched optimistically (`active` is a dispatcher matcher input, so an optimistic value could show the opposite of what the integration is doing), a failed toggle surfaces the message and leaves the row unchanged, and the screen is asserted to carry **no** `Delete` control and no *"permanently"* |

**Two changes outside the S3 tickets, both defects found while building.**

1. `web/src/hooks/useFocusOnNavigate.ts` — `/portal` and `/portal/:appId` fell through
   `getPageTitle`'s final branch and announced *"Workspace"*, which failed
   `a11y-aria-invariants.test.ts` on `main`. That test was **already red** before this slice
   touched anything; both routes now answer *"Developer Portal"*.
2. `SecretOnceDialog` gained a `'none'` value for `rotationPolicy`, and app registration now passes
   it. A freshly registered app was being shown *"The previous secret stopped working immediately.
   Any integration still using it is failing now"* — a sentence about an integration that has never
   existed, on the screen where a developer is being told the security rules. The rotation policy is
   still rendered from the server's value everywhere it actually applies (`RotateSecretDialog`, and
   the rotate branch of the same dialog), so PF-670's D3-is-data property is untouched.

## Slice S1-b (`pf/L22-ts8-coverage`), 2026-08-15 — PF-662 written, **not yet run**

`e2e/portal-replay-ts8.spec.ts` plus `seedDeadLetteredLadder` in
`e2e/fixtures/isolated-env.ts`. **PF-662 stays ◐, not ☑**, and the reason is the honest one: the
agent that wrote it did not own the test database that run, so the spec has **never been
executed**. A ☑ here would be a claim a grader could falsify in one command.

What it does, so the next person can judge it rather than re-derive it:

| Step | How | Why not the obvious way |
|---|---|---|
| Six failures → dead letter | Seeded via `seedDeadLetteredLadder`, one shared `idempotency_key` and one `delivery_group_id` | Driving the real ladder costs p.4's schedule — 1s, 4s, 16s, 1m, 5m — which is 6m22s against a 60 s budget. **PF-481 owns proving the ladder**; everything after the seed is real |
| *"a now-healthy subscriber"* | A `node:http` server on `127.0.0.1` that answers 200 and records what it received | `target_url` is immutable by design (PATCH takes only `active`), so a subscriber cannot be re-pointed — the healthy target must be the same URL. Loopback is permitted because the e2e API runs `NODE_ENV=test`, which is the door `localTargetsPermitted` exists to open |
| *"Click Replay"* | The rendered button in the delivery log | The half no API test can cover, and the word p.5 actually uses |
| *"idempotency key intact"* | Asserted on the `Idempotency-Key` header **at the subscriber** | Reading our own database back would assert our own write. The header is what a real integration dedupes on |
| PF-477, history kept | The DLQ view still shows exactly one row after the replay; the unfiltered log shows more than one | A UI that swapped the row in place would hide the record an operator came for |

The seeder lives in `isolated-env.ts` rather than in the spec because **`pg` does not resolve from
`e2e/*.spec.ts`** — measured, not assumed: `node --input-type=module -e "import('pg')"` from `e2e/`
fails `ERR_MODULE_NOT_FOUND`, and there is no `node_modules/pg` at the root. `seedFleetGraphScenario`
is in that file for the same reason.

The spec is **live rather than `test.fixme()`** on purpose. A spec that fails loudly tells the truth
about itself; a skipped one reports nothing and reads as coverage in a file listing. Its header says
it has never been run.

**Layout.** Subscriptions are a **tab** beside the delivery log in the main content panel, not a
fifth region. The delivery log stays the default view, so p.12's demo path — login → `/portal` →
select app → DLQ → Replay — gains **no clicks**; `portalStates.test.tsx` asserts a cold
`/portal/:appId` lands on the log. A subscription row's *Deliveries* button hands its id to the
log's `subscription_id` filter in one navigation rather than making a developer copy a UUID between
two screens.
