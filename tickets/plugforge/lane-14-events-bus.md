...
> **Board reconciled 2026-08-13 (PF-029).** Every ticket above was marked in one pass, not
> individually re-verified. Basis: all four slice branches (`pf/L14-event-registry`,
> `pf/L14-event-bus`, `pf/L14-domain-publish`, `pf/L14-publish-fitness`) are ancestors of
> `pf/integration`, and each slice's artifact exists and its tests pass in the suite —
> `EVENT_TYPES` frozen in `webhooks/events.ts`, `IEventBus` in `webhooks/bus.ts`, the Liskov
> suite in `busContract.ts`, `bus.publish(` call sites in `services/documents.ts` and
> `services/sprints.ts`, and `publishFitness.test.ts`. The lane built the work and did not
> update its own board; this closes that gap rather than claiming fresh verification.
> One known staleness: PF-408's title still says *identifiers only, never document content*,
> which is the opposite of what D7 decided — the lane itself made that call and recorded it
> in L99. The title is wrong; the shipped behaviour follows D7.
