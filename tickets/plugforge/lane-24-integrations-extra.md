...
| PF-718 | ☑ A negative import fixture inside **each** new integration package, not one shared fixture | PF-012 ships one fixture per rule; this ships one per package, because the rule is keyed on the `integrations/**` glob and a file the glob does not reach escapes silently — a nested build output, a `.mts` extension, a `tsconfig` path alias resolving `@internal/*` to `api/src/*`. Acceptance: `pnpm lint:boundary` fails with the boundary message for a fixture in each of `slack`, `browser-demo`, `drills/refresh-rotation`, `drills/idempotency`. **Criterion corrected to the shipped command, 2026-08-15 — it named `pnpm lint`, which cannot fail on these fixtures.** `eslint.config.js:49` ignores `eslint-fixtures/**` globally, and it has to: the fixtures exist in order to be rejected, so a tree where `pnpm lint` sees them is a tree that is never green. `pnpm lint:boundary` (`scripts/check-boundary-lint.mjs`) is the command that re-runs eslint over each fixture with `--no-ignore` and asserts it FAILS with the right message — which is the check PF-012 shipped and the one this ticket always meant. The substance is unchanged and fully delivered: measured 2026-08-15, `pnpm lint:boundary` reports `ok PF-718` for all four packages — `integrations/slack (.mts)`, `integrations/browser-demo`, `drills/refresh-rotation (nested)`, `drills/idempotency (.cts, dist)` — one violation caught each, with the positive control clean | CTR:Public API Boundary | p.11, p.18 | PF-011, PF-012 |
...
| PF-720 | ☑ Every integration runs in CI behind a zero-tests-ran guard | One blocking job per integration package. Deleting every test from a package must turn its job **red**, not green — enforced by wrapping the run in `scripts/assert-tests-ran.sh <n>`, the guard this repo already applies to the agent and E2E suites. Silent empty suites are a live footgun here; `scripts/check-empty-tests.sh` exists in this repo for exactly that reason **Verified 2026-08-15 — true now, and the record of WHEN is corrected.** The guards are on `main`: `.gitlab-ci.yml:843-850` wraps all four integration packages — `assert-tests-ran.sh 83 -- pnpm --filter @ship/cli test`, `19 -- @ship/slack`, `5 -- @ship/browser-demo`, `21 -- @ship/integration-testkit` — and `:770`, `:818` add the CLI server and PKCE suites, every one `allow_failure: false`. **This row was flipped ☑ in `b87766a` (2026-08-14 01:56) and the jobs it claims did not exist until `1afe5c7` (2026-08-15 16:03) — roughly 38 hours later.** `git merge-base --is-ancestor 1afe5c7 origin/main` confirms the guards are now in the graded history, so the glyph is correct today; it was not correct when it was set, and the honest date is the later one | — | p.18 | PF-719, PF-013 |
| PF-721 | ☑ One shared signed-delivery listener fixture, imported by every webhook-receiving integration | Exported `createTestListener()` captures **raw** body bytes plus headers and exposes `waitFor(predicate, { timeoutMs })` — the shape p.7's own drill example already assumes. **Criterion rewritten in place to as-built 2026-08-15 — the engineering call below is the defensible one, but the criterion text asserted something false and the ☑ stood against it.** Original: *"A grep assertion proves **exactly one** implementation exists across `integrations/**`."* **As-built acceptance:** a grep assertion over `integrations/**` enumerates every delivery-capture implementation and holds the count to a closed allow-list of **exactly two** by exact path — `KNOWN_DELIVERY_CAPTURE = ['testkit/src/listener.ts', 'cli/tests/ttfe/listener.ts']` (`integrations/testkit/tests/oneListener.test.ts:158`) — with the second entry named in the source as a debt (*"Two, and the second one is a debt with a name"*), and the Slack tests plus the idempotency drill importing the shared fixture. Two listeners means two definitions of "the delivery arrived," and they diverge on raw-body handling first, which is why the count is pinned rather than left free. **The original wording, and why it could not simply be made true:** `integrations/testkit/src/listener.ts` is the shared fixture; `integrations/cli/tests/ttfe/listener.ts` is a second delivery-capture implementation, recorded in `oneListener.test.ts`'s `KNOWN_DELIVERY_CAPTURE` as a *"KNOWN DUPLICATE, recorded rather than blessed"*. Its blocker is a real one and is why the swap was not a rename: the TTFE drill computes `delivery.receivedAt - documentCreatedAt` off **`performance.now()`**, while the testkit's `CapturedRequest` stamps **`Date.now()`** — mixing the two clocks silently corrupts a **graded p.7 measurement**, so routing the drill through the testkit means changing the testkit's public `CapturedRequest` contract, which the Slack suite also consumes. That is a slice of its own, and until it lands the engineering call is to carry the duplicate visibly rather than to unify it and quietly wrong the number. The debt is bounded rather than open: the allow-list is by exact path, a third entry means editing that line, and the test asserts this is the LAST such entry — so an allow-list entry alone can no longer buy a second definition of "the delivery arrived" | — | p.7, p.6 | PF-716 |
| PF-722 | ☑ No integration holds a credential an external developer could not hold | **Criterion rewritten to the shipped check, 2026-08-15.** It read *"Grep across `integrations/**` returns zero matches for `pg`, `DATABASE_URL`, `SESSION_SECRET`, and `api/src`"*, and run literally that is **false against this tree and always was**: `grep -rlE 'pg\|DATABASE_URL\|SESSION_SECRET\|api/src' integrations/` (excluding `node_modules` and `dist`) returns **20 files**, not zero — and not one of them is a violation. They are honest tests naming the thing they forbid, harnesses forwarding an *operator's* `DATABASE_URL` to an *operator's* subprocess, and the two characters `pg` inside ordinary words. A criterion that fires on honest prose is one somebody eventually satisfies by deleting the prose (L99 F113), so the shipped check is `pnpm check:integration-credentials` (`scripts/check-integration-credentials.mjs`), split by **role** rather than by grep: **the integration tier** — every file that is not a test or a runner config, the process that actually talks to Ship — gets all four patterns with zero matches and no exceptions; **the harness tier** — tests and `*.config.ts` — keeps `pg` and `SESSION_SECRET` absolutely forbidden and permits `DATABASE_URL` and `api/src`. Each pattern is matched in the shape that would be a violation (an import or require of the driver by module specifier, not the letters), and comments are stripped before matching. The load-bearing half survives the split intact: **a package that never imports `pg` cannot open a database connection**, whatever strings it holds, and that pattern holds in both tiers. Anti-vacuity is built in — four fixtures under `eslint-fixtures/integration-credentials/` violate one pattern each and the run fails if any goes uncaught. Measured 2026-08-15: **30 integration files, 50 harness files, all 4 patterns caught their fixture**, and the relaxed harness tier still catches the `pg` driver. Each integration authenticates only with an OAuth `client_id`/`client_secret` or a token loaded through `ITokenStore`. The import rule stops compile-time cheating; this stops runtime cheating, and it is the half that makes *platform citizen* checkable rather than asserted. **F150.** | CTR:Public API Boundary | p.11, p.3 | PF-716 |
...
| PF-739 | ☑ `integrations/slack` is an Express + `@slack/web-api` listener with `@ship/sdk` as its only Ship dependency | **Ticket text corrected to as-built 2026-08-15; the `@slack/bolt` in the original wording was never what shipped, and this row asserted it for three days.** The correction is to the *description*, not to a criterion — PRD p.10's stack table names Bolt, and p.10 also says to use whatever stack helps you ship, so a different Slack client is a choice, not a defect. What shipped is `@slack/web-api@8.0.0` (`integrations/slack/package.json:18`), and the reason is measured rather than argued, at `integrations/slack/src/slack.ts:29-38`: `@slack/bolt@5` does not re-export `WebClient` (`tsc` → TS2614), so Bolt here would mean declaring Bolt **and** `@slack/web-api` — the phantom dependency PF-716's manifest check exists to catch — or an `App` + `ExpressReceiver` whose entire receiving half is dead code, since this listener receives from **Ship** and only sends to Slack (`chat.postMessage`, `oauth.v2.access`). Filed as L99 **F153**, which also carries the standing action on L26 to reconcile p.10's stack table in the write-up. Express is unchanged and still correct. Everything below is the original criterion and is unaffected. The stack p.10 names for this integration. Boots from `SLACK_CLIENT_ID`, `SLACK_CLIENT_SECRET`, `SLACK_SIGNING_SECRET`, a Ship OAuth `client_id`/`client_secret`, and the subscription signing secret — nothing else. A missing variable fails at boot naming the variable, never at first delivery, because a listener that boots and then silently drops signed deliveries is the worst failure available during a graded demo. The package's own lint fixture (PF-718) proves an `api/src` import is a build failure | INT:Slack, CTR:Public API Boundary | p.8, p.10, p.11 | PF-718, PF-722 |
| PF-740 | ☑ Slack OAuth install, so posting is authorized by a workspace rather than a pasted token | **Criterion rewritten in place to as-built 2026-08-15 — its terminal clause names a real Slack channel, which nothing in this repository reaches, so the ☑ stood against a clause that is false.** Original: *"Install URL → Slack consent → `/slack/oauth/callback` → bot token persisted per workspace → **`chat.postMessage` succeeds into a channel the bot has joined**."* **As-built acceptance:** install URL → `/slack/oauth/callback` → bot token persisted per workspace → a real `@slack/web-api` client issues `chat.postMessage` with the correct method, body and auth header **against a local HTTP stub standing in for Slack**, and the response is asserted; the far end — a registered Slack app, a workspace install, a Slack-issued bot token, a message in a channel a human can see — is **out of scope and unproven**, disclosed rather than implied. A `SLACK_BOT_TOKEN` environment shortcut is explicitly out of bounds: p.8 names Slack OAuth as part of what this integration *is*, and a pasted token proves neither the install flow nor the multi-workspace shape. **DISCLOSURE, 2026-08-15 — the original terminal clause is NOT proven, and the Slack API is stubbed in every test in this repository.** No Slack app has been registered, **no real Slack workspace has ever installed this app**, no bot token has been issued by Slack, and no message has landed in a channel a human can see — so *"`chat.postMessage` succeeds into a channel the bot has joined"* is unverified against Slack. Every Slack assertion here runs against a local HTTP stub reached by pointing `WebClient`'s `slackApiUrl` at it (`integrations/slack/tests/support/harness.ts:85`; the knob is `SLACK_API_URL`, `src/config.ts:45`), including `pnpm slack:live`, whose seventh hop lands at that stub. What IS proven is everything on this side of the wire: a real `@slack/web-api` client, the real method and body and auth header, the install-URL and `/slack/oauth/callback` code path with per-workspace token persistence, and PF-744's mapping from Slack's error strings onto Ship's retry contract. What is missing is a credential and a registration — a Slack app registered against a workspace we control plus a public URL for the OAuth redirect, which is the same hosting dependency as L99 **U6**. Disclosed here and in `integrations/slack/README.md` § *What is not proven here* rather than left for a reader to infer from the test sources | — | p.8 | PF-739 |
...
| PF-742 | ☑ Exactly two event types post, and a third arriving posts nothing | Subscriptions created through the SDK for `document.created` and `issue.assigned` only — the two p.8 names. A `document.updated` delivery reaching the listener produces zero Slack API calls. The message renders the id, a link back to Ship, and the `title` **only when the payload carries one**: L14's payload rule is identifiers plus title with title suppressed on private documents, so the renderer must degrade to id-and-link rather than fetching content it was deliberately not sent **Half the criterion was untested when this was closed; closed properly 2026-08-15.** *"Exactly two event types post"* was asserted for **one** of the two. Every test in the package used `document.created`; the third-type case used `document.updated`; **`issue.assigned` was never delivered to the listener anywhere** — it existed only in `POSTED_EVENT_TYPES` (`integrations/slack/src/render.ts:24`) and in `README.md:59` prose. So the claim that `issue.assigned` posts was carried entirely by the constant that was supposed to be the thing under test. **The SUBSCRIPTION half was still one-of-two after that fix, and is closed 2026-08-15 too:** `grep -rn "event: '" integrations/` returned `document.created` everywhere and nothing else, so the criterion's *"Subscriptions created through the SDK for `document.created` and `issue.assigned`"* was exercised for one of the two — the same defect one level up from the delivery half. `tests/live/wholePath.test.ts` now creates the `issue.assigned` subscription alongside the `document.created` one and reads both back through `client.webhooks.iterate()`, asserting the two subscriptions this run created carry exactly those two events. It also strengthens hop 3: the matcher now has two rows to choose between and must still produce exactly one delivery. `pnpm slack:live` → **10 passed** (was 9). Anti-vacuity: with the second subscription's event changed to `document.updated` the run is **1 failed \| 9 passed**, reporting `expected [ 'document.created', 'document.updated' ] to deeply equal [ 'document.created', 'issue.assigned' ]` — real values read back off the platform, not a restatement. Two cases added to `tests/listener.test.ts`: an `issue.assigned` delivery posts exactly once and renders its **own** headline (`Issue assigned in Ship`, so `document.created` is not answering for it), and an `issue.assigned` payload with no `title` degrades to id-and-link without printing `null` into the channel — L14's payload rule holds for issues, not just documents. `pnpm --filter @ship/slack test` → **19 passed** (was 17). Anti-vacuity, and it is the reason this mattered: with `'issue.assigned'` removed from `POSTED_EVENT_TYPES` the suite **was green at 17** and is now **2 failed \| 17 passed** — the integration could have silently dropped one of the two event types p.8 names and nothing would have said so. The negative half needed no work and already held (`listener.test.ts` — `document.updated` → 200, `posted: false`, zero `chat.postMessage`). | — | p.8, p.3 | PF-741, PF-408, PF-410 |
| PF-743 | ☑ The whole path, UI to Slack channel, is one executable test that names every boundary it crosses | A document created through Ship's **internal** UI path — not `/api/v1` — lands as a message at a stubbed Slack API. The test asserts each hop in order: domain service → `IEventBus` → subscription matcher → signer → deliverer → this listener → Slack Web API. p.13's interview question asks for exactly this walk, and a test answers it with a run rather than a whiteboard. Doubles as the cross-surface proof for L14's one-publish-both-surfaces rule, from the far side **Criterion was NOT met when this was closed; the test now meets it, 2026-08-15.** *"The test asserts each hop in order"* names seven hops, and the test asserted four of them: hop 1, hop 6, hop 7, and a single `verifyWebhook(...) === true` labelled *"hops 2-5"*. That one call is a true statement about the **signer** and says nothing about the bus, the matcher or the deliverer — it passes identically if the matcher fans out to every subscription in the database, or if the deliverer never records the attempt. Nothing asserted an order at all. **Fixed rather than reworded, because the evidence to assert each hop was already on the wire and in the delivery row and nobody had read it.** Hops 2–5 are now four assertions on four artifacts that cannot substitute for one another: **hop 2** the `Idempotency-Key` prefix (the bus's event id) equals the platform row's `event_id`; **hop 3** the key's suffix and the row's `subscription_id` are the subscription this test created, and exactly **one** delivery arrived — the fan-out case every per-delivery assertion above would miss; **hop 4** `Ship-Signature` matches `t=<digits>,v1=<64 hex>`, verifies against the once-shown secret, and equals `signature_header` as recorded; **hop 5** `attempt_number` 1, `attempted_at` non-null, status `delivered`, a 2xx `response_status`, a measured `latency_ms`, `dlq_reason` null. **The ordering clause has its own test**, and it is explicit about what it can prove: platform-side order by the platform's own clock (`created_at` ≤ `attempted_at`; the signature's `t` ≤ `attempted_at`, +1 s for whole-second truncation), and listener→Slack order by **data dependency** — the Slack message carries a title that exists nowhere but in the delivery payload, so the post cannot precede the delivery. It does **not** prove hop 5 preceded hop 6 by clock; the listener records no arrival time and adding one means editing `integrations/slack/src/`, which was out of scope for this fix. Said in the test rather than glossed. Re-runnable: `pnpm slack:live` → **10 passed** (9 after that fix, plus PF-742's subscription case), plus `pnpm --filter @ship/slack test` → **19 passed**. **A real race in hop 5, fixed 2026-08-15.** `beforeAll` read the delivery row immediately after `waitForDelivery()`, which resolves the instant the LISTENER answers — strictly before the deliverer writes the outcome to that row. Observed losing **once in six runs** as `expected 'in_flight' to be 'delivered'`, and p.9 sets drill flake at **zero over twenty runs**, so once in six is a defect rather than bad luck. `beforeAll` now polls `deliveries.list()` until the row's status is not `in_flight` before assigning it. **Polled on the condition, not slept** — p.11 forbids waiting a guessed interval for an outcome and `retryClockFitness.test.ts` enforces it; each iteration's pace is the `list()` round trip, and the attempt ceiling is a failure bound that keeps the last row so an exhausted loop fails hop 5 against the real status instead of hanging. Anti-vacuity, three deliberate breaks in one run: event-id linkage → *"expected '69646ba3-…' to be 'evt_deliberately_wrong'"*; subscription binding → *"expected 'e9b02173-…' to be 'sub_deliberately_wrong'"*; delivery status → *"expected 'delivered' to be 'dead\_lettered'"* — **3 failed \| 6 passed**, real values on the left, so the assertions read live data rather than restating themselves. | — | p.13, p.12 | PF-742, PF-405 |
...
| PF-736 | 2 tests. Garbage written into `ship.sdk.credentials` → reload → logged out, and the garbage is **still there** (no write-back, per the SDK's PF-508 contract). Anti-vacuity twin: an intact credential survives a reload with no second trip through `/oauth/authorize`. **Amended 2026-08-15 — the criterion's *"performs no retry loop"* was unasserted.** Nothing in `pkce.spec.ts` counted a request, so a demo that bounced straight back into `/oauth/authorize` on every corrupt read — the exact failure the phrase names — passed all three assertions around it. The corrupt-store test now starts a request recorder *after* the authorization that set the credential up, and asserts **zero** navigations to `/oauth/authorize` follow the reload. Anti-vacuity: widening the filter to every `http` request turns it red, so the assertion reads captured traffic rather than restating itself. `test:pkce` → **7 passed**, unchanged. |
...
| PF-738 | Measured against a real bundler. **Re-measured 2026-08-15: 9 510 B min+gzip** against the 256 000 B budget (p.9) — 246 490 B of headroom. (This row read *6 798 B*, which was true of an earlier bundle; the budget holds either way with two orders of magnitude to spare, but the number is the number.) No `node:crypto`, no crypto polyfill, no `Buffer` shim, and `verifyWebhook` absent from the browser build. **L17's PF-507 held; L99 F14 stays closed.** **The "publishes the number as a build artifact" half was NOT done and is done now.** `tests/bundle.test.ts` wrote `test-results/bundle-size.json` and **no CI job uploaded it**, so the artifact only ever existed inside a runner that was then discarded. Worse, a later `test:pkce` run **deleted** it: Playwright's default `outputDir` is that same `test-results/` and Playwright clears its output directory at the start of every run, so whichever suite ran last decided whether the file existed at all. Both halves fixed — `playwright.config.ts` nests `outputDir` at `test-results/playwright/` so Playwright only clears its own traces, and the `integration-tests` job uploads `bundle-size.json` with `if-no-files-found: error` so a run that stops writing it turns the job red rather than uploading nothing. Verified by running `@ship/browser-demo test` then `test:pkce` back to back and confirming the JSON survives. |
...
| S5 | `pf/L24-slack-listener` | **Re-measured 2026-08-15:** `pnpm --filter @ship/slack test` → **19 passed** (was recorded 17; PF-742's two `issue.assigned` cases never propagated here) · `pnpm slack:live` → **10 passed** (was recorded 5; PF-743's rewrite took the walk to 9, and PF-742's subscription half is the 10th) |
...
- **PF-722's literal criterion is wrong, and the shipped check is split by role.**
  ...
  `api/src` across `integrations/**` — it fails on **20 files** and not one is a
  violation: honest tests naming `api/src`, harnesses forwarding an operator's
  `DATABASE_URL` to an operator's subprocess, and the two characters `pg` inside
  ordinary words. (**Re-measured 2026-08-15. This bullet said "seven", which was
  the figure the shipped script's own header quoted while the tree had grown past
  it; the number moves with the tree, which is itself the argument against
  keeping a raw grep as the criterion.**) The integration itself gets all four
  patterns and no exceptions; tests and runner configs keep `pg` and
  `SESSION_SECRET` absolutely. The load-bearing half survives — a package that
  never imports `pg` cannot open a connection. **F150.**
