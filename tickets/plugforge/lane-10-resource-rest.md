...
**S1 shipped on `pf/L10-me` (commit `070c915`). S2 shipped on `pf/L10-issues`.**

| Slice | Tickets | State |
|---|---|---|
| S1 `pf/L10-me` | PF-271–276 | **Done**, except PF-276 (**blocked on L05** — no device grant exists). PF-275 closed 2026-08-15 on `pf/L16-L10-L23-tail`: `src/platform/api/v1/me/` **36/36**, `list: false` and both observable criteria each pinned by a named test |
| S2 `pf/L10-issues` | PF-277–283 | **Done.** Four routes, one projection, a keyset that survives a PATCH, and cross-workspace + cross-`document_type` isolation. 71 tests across 5 files |
| S3 `pf/L10-sprints` | PF-284–289, PF-291 | **Done.** Four routes over a resource with **no internal list to extract**, honest derived fields, and the public producer `sprint.completed` never had. 83 tests across 5 files |
| S4 `pf/L10-event-callsites` | PF-290–293 | **Done, distributed across S2 and S3** — see the note below. PF-290 was **already done by L14** and S3's job was not to break it; PF-292/293 landed with S2; PF-291 landed with S3 |
| S5 `pf/L10-parity-and-budget` | PF-294–296 | **PF-294 done and asserted** — now over TEN operations rather than four, with zero lines changed under `platform/openapi/`. PF-296(a)'s 3×3 scope matrix landed with S3. PF-295's enumeration count and PF-296(b)'s query budget are **not done** |

**Why PF-292/293 landed in S2 rather than S4, stated plainly.** They are call sites
*inside* `issueService.create` and `issueService.update` — the functions S2 had to
write anyway. Splitting a function across two branches so the ticket numbers line
up with the slice table would have meant shipping S2 with a write path nobody had
tested for the thing it is supposed to emit. The slice boundary moved; the tickets
did not.

**Two S4 findings that change what remains.** Both verified by reading the files,
not inferred:

1. **PF-290 is already done, by L14.** `api/src/services/sprints.ts` exists and is
   the transition state machine `SPRINT_TRANSITIONS` plus the publish for
   `sprint.started` and `sprint.completed`, and `routes/weeks.ts` already calls
   `sprintService.start` / `sprintService.transition`. Re-doing it would be a
   second publish site, which is the exact defect PF-290 exists to prevent. What
   S3 owes is the *consumer*: the public sprints module must resolve through that
   service and contain no `.publish(` of its own.
2. **PF-291's remaining half is a public write path, not a producer.** F9's
   closure by L14 means `sprint.completed` CAN fire and is tested at the domain
   level. What has no public producer is the transition — which is
   `PATCH /api/v1/sprints/:id` accepting `{status:'completed'}`, and that is S3's.
