...
| PF-224 | ☑ `next_cursor` is present-and-`null` on the last page, never omitted | Assertion is `'next_cursor' in body && body.next_cursor === null`, not `body.next_cursor == null` — an omitted key and an explicit null are different to a typed SDK consumer. Boundary case: exactly 25 rows at page size 25 returns `next_cursor: null` in **one** request, no phantom empty final page. Implementation fetches `limit + 1` to decide | TS-4 | p.3 | PF-223 |
