...
| PF-514 | ☐ Install footprint: zero production dependencies, measured at **< 250 KB min+gzip** | p.9 sets the target: *"SDK install size (production deps only)"* at *"< 250 KB minified + gzipped"*. A committed script measures the packed tarball's production closure (bundle `dist` + every transitive `dependencies` entry, minify, gzip, sum) and writes the number to a report. Acceptance: `dependencies` is empty **and** the measured number is recorded — the empty-deps assertion is the mechanism, the byte count is the proof. Deliberately measures the closure and not just `dist`, so adding one 400 KB dependency fails even though `dist` did not grow | — | p.9, p.15 | PF-496 |
