 ...
 *   - DATABASE_URL, SESSION_SECRET, CORS_ORIGIN, WEBHOOK_SECRET_KEY
...
export async function loadProductionSecrets(): Promise<void> {
  ...
  // PF-625 / L99 F91 — WEBHOOK_SECRET_KEY joins the REQUIRED group, not the
  // tolerant one below.
  //
  // It is the AES-256-GCM key for every webhook subscription's signing secret
  // (L15 PF-422). `api/src/deps.ts` resolves it LAZILY on purpose, so a
  // deployment without it boots green, passes /health, and then throws
  // `WebhookSecretCryptoError` out of the FIRST `POST /api/v1/webhooks` --
  // which through the SDK reaches the caller as a bare `server_error`. That is
  // three layers from the cause, and it is the exact command PRD p.12's demo
  // script runs.
  //
  // Why REQUIRED and not tolerant like the three app secrets: those seed DATA,
  // and taking the whole API down over a missing OAuth seed trades a degraded
  // deployment for no deployment. This is INFRASTRUCTURE -- the direct
  // analogue of SESSION_SECRET, which is already in this group. Both are
  // generated by Terraform in ssm.tf in the same apply that creates
  // DATABASE_URL, so an absent one does not mean "that feature is off", it
  // means the environment was never provisioned. Failing here names the
  // variable at boot instead of leaving the discovery to a grader.
  //
  // This only affects the Elastic Beanstalk path. Any platform that sets
  // DATABASE_URL itself (Render, a container runtime, CI) returns above and
  // never reaches this line.
  const [databaseUrl, sessionSecret, corsOrigin, cdnDomain, appBaseUrl, webhookSecretKey] =
    await Promise.all([
      ...
      getSSMSecret(`${basePath}/WEBHOOK_SECRET_KEY`),
    ]);
  ...
  process.env.WEBHOOK_SECRET_KEY = webhookSecretKey;
